...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        with init_context(), warnings.catch_warnings():
            ...
            # Get attn_implementation from override_model_config, default to 'eager' for Colab compatibility
            attn_impl = override_model_config.get('attn_implementation', 'eager')
        ...
        if role == 'actor' and optim_config is not None:
            from verl.utils.torch_functional import get_constant_schedule_with_warmup, get_linear_schedule_with_warmup_and_decay
            ...
            lr_decay = optim_config.get('lr_decay', False)
            num_warmup_steps = int(num_warmup_steps_ratio * total_steps)

            print(f'Total steps: {total_steps}, num_warmup_steps: {num_warmup_steps}')

            if lr_decay:
                actor_lr_scheduler = get_linear_schedule_with_warmup_and_decay(optimizer=actor_optimizer,
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            if data.meta_info.get('return_entropy', False):
                output = DataProto.from_dict(tensors={'old_log_probs': output[0], 'entropy': output[1]},
